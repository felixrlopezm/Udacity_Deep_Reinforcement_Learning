...
#
#
# Smart Agent built with Deep Deterministic Policy Gradient (DDPG) algorithm
#
# Smart agent's code based on that described in the DDPG paper and modify accordingly to solve
# the task at hand.
#
...
from actor_critic_models import Actor, Critic
...
class Agent():
    ...
    def __init__(self, state_size, action_size, hyperparameters):
        ...
        self.critic_local = Critic(state_size, action_size, self.RANDOM_SEED, self.FC1, self.FC2).to(device)
        self.critic_target = Critic(state_size, action_size, self.RANDOM_SEED, self.FC1, self.FC2).to(device)
        ...
        # Replay memory
        self.memory = ReplayBuffer(action_size, self.BUFFER_SIZE, self.BATCH_SIZE, self.RANDOM_SEED)

    def step1(self, state, action, reward, next_state, done):
    ...
    def step20(self, states, actions, rewards, next_states, dones, t):
        """Save experience in replay memory, and use random sample from buffer to learn."""
        # Save experience / reward    
